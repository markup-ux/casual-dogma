...
namespace Arrowgene.Ddon.Cli
{
    internal class Program
    {
        ...
        private void LoadCommands()
        {
            ...
            AddCommand(new ErrorCommand(_errorMonitor));
        }
        ...
        private void RunArguments(string[] arguments)
        {
            ...
            _errorMonitor.SetLogDirectory(_logDir);
            ...
            {
                Logger.Info("Type a command to run it (e.g. 'errors'), or 'e' / 'exit' and press enter to quit.");

                // ReadLine returns null at EOF (e.g. no interactive console / redirected stdin),
                // which falls through to shutdown instead of busy-looping.
                string input;
                while ((input = Console.ReadLine()) != null)
                {
                    ...
                    CommandParameter lineParameter =
                        ParseParameter(input.Split(CliSeparator, StringSplitOptions.RemoveEmptyEntries));
                    if (lineParameter == null)
                    {
                        continue;
                    }

                    if (ProcessArguments(lineParameter) == CommandResultType.Exit)
                    {
                        break;
                    }
                }
            ...
            // Shut down every command that was loaded. Commands run interactively replace
            // _lastCommand, so shutting down all of them ensures long-running commands such as
            // 'server' are always stopped regardless of what ran last.
            foreach (ICommand command in _commands.Values)
            {
                ...
                {
                    Logger.Error($"Error shutting down command '{command.Key}': {ex}");
                }
            }

            Logger.Info(_errorMonitor.BuildSessionSummary());
        ...
        private void LogProviderOnLogWrite(object sender, LogWriteEventArgs e)
        {
            ...
            // Record and persist errors before any console-only filtering below.
            _errorMonitor.Inspect(log);
