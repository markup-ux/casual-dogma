public class ChatCommand : IChatCommand
{
    ...
    public override void Execute(DdonGameServer server, string[] command, GameClient client, ChatMessage message, List<ChatResponse> responses)
    {
        ...
        // The client only acts on the progress notification below if it already knows about the
        // warp quest. That registration is normally only delivered on area entry, so re-send the
        // set-quest-list here to make the command self-contained.
        client.Send(Arrowgene.Ddon.GameServer.Handler.QuestGetSetQuestListHandler.BuildQuestListNtc(client, client.Character.AreaId));
