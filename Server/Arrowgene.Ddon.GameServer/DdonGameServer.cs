...
namespace Arrowgene.Ddon.GameServer
{
    public class DdonGameServer : DdonServer<GameClient>
    {
            ...
            : base(ServerType.Game, setting.ServerSetting, database, assetRepository)
        {
            ...
            RecoverableHpManager = new RecoverableHpManager(this);
            ...
            SupplyCacheManager = new SupplyCacheManager(this);
        ...
        public RecoverableHpManager RecoverableHpManager { get; }
        ...
        public RevivalManager RevivalManager { get; }
        ...
        public SupplyCacheManager SupplyCacheManager { get; }
        ...
        protected override void ClientDisconnected(GameClient client)
        {
            ...
            PersistLogoutLocation(client);

            RecoverableHpManager.OnCharacterDisconnected(client);

            if (client.Character != null)
            {
                RevivalManager.UnloadCharacter(client.Character.CharacterId);
            }
        ...
        /// <summary>
        /// Records the stage the character is currently in so that, on their next login, they can be
        /// resumed where they logged out instead of being sent to the last visited safe area.
        /// Only applies to the normal game mode; instanced modes such as Bitterblack Maze are skipped
        /// so players are never dropped back into an instance that no longer exists.
        /// </summary>
        private void PersistLogoutLocation(GameClient client)
        {
            if (!GameSettings.GameServerSettings.EnableReturnToLogoutLocation)
            {
                return;
            }

            if (client.Character == null
                || client.GameMode != Shared.Model.GameMode.Normal
                || client.Character.Stage.Id == 0)
            {
                return;
            }
            ...
            try
            {
                client.Character.LastLogoutStageId = client.Character.Stage.Id;
                Database.UpdateCharacterBaseInfo(client.Character);
            }
            catch (Exception ex)
            {
                Logger.Error(client, $"Failed to persist logout location: {ex}");
            }
        }
        ...
        private void LoadPacketHandler()
        {
            ...
            AddHandler(new InstanceGetDropItemSetListHandler(this));
