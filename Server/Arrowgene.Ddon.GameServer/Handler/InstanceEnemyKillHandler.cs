    ...
    public class InstanceEnemyKillHandler : GameRequestPacketHandler<C2SInstanceEnemyKillReq, S2CInstanceEnemyKillRes>
    {
        ...
        public override S2CInstanceEnemyKillRes Handle(GameClient client, C2SInstanceEnemyKillReq packet)
        {
            ...
            if (enemyKilled.RepopCount > 0 && enemyKilled.RepopNum < enemyKilled.RepopCount)
            {
                ...
                Server.LevelSyncManager.TryApplyEnemyLevelSync(client, stageId, enemyKilled);

                S2CInstanceEnemyRepopNtc repopNtc = new S2CInstanceEnemyRepopNtc()
                {
                    LayoutId = layoutId,
                    WaitSecond = ExplorationRepopHelper.ResolveRepopWaitSeconds(_gameServer, enemyKilled),
            ...
            Server.Database.ExecuteInTransaction(connectionIn =>
            {
                ...
                {
                    bool autoLoot = Server.GameSettings.GameServerSettings.EnableAutoLoot;
                    ...
                    foreach (var partyMemberClient in client.Party.Clients)
                    {
                        ...
                        bool isGoldenBag = instancedGatheringItems[typeof(EnemyEpitaphRoadDropGenerator)].Any();

                        if (autoLoot)
                        {
                            // Auto-loot: hand the rolled drops straight to the player. Coin/rift/orb pouches
                            // are routed through ItemManager.GatherItem -> HandleSpecialItem -> WalletManager,
                            // so the gold settings (e.g. ConvertGoldRewardsToExp) still apply.
                            var droppedItems = instancedGatheringItems.Values.SelectMany(x => x).Where(x => x.ItemNum > 0).ToList();
                            ...
                            foreach (var dropItem in droppedItems)
                            {
                                queuedPackets.AddRange(Server.ItemManager.GatherItem(partyMemberClient, autoLootNtc, dropItem, dropItem.ItemNum, connectionIn));
                            }
                            ...
                            // Anything that didn't fit (e.g. full inventory) still drops as a bag to pick up manually.
                            var leftovers = droppedItems.Where(x => x.ItemNum > 0).ToList();
                            if (leftovers.Any())
                            {
                                uint leftoverSetId = partyMemberClient.InstanceDropItemManager.Assign(layoutId, packet.SetId, leftovers);
                                partyMemberClient.Enqueue(new S2CInstancePopDropItemNtc()
                                {
                                    LayoutId = packet.LayoutId,
                                    SetId = leftoverSetId,
                                    MdlType = (byte)(isGoldenBag ? 1 : (enemyKilled.DropsTable?.MdlType ?? 0)),
