...
namespace Arrowgene.Ddon.GameServer.Quests
{
    public class GenericQuest : Quest
    {
        ...
        // When enabled, item turn-in/delivery quest steps auto-complete from the player's bag
        // (HaveItemAllBag) instead of requiring an NPC hand-in. Captured at quest load time from
        // the AutoCompleteTurnInQuests server setting.
        private static bool AutoCompleteTurnIns = false;

        public static GenericQuest FromAsset(DdonGameServer server, QuestAssetData questAsset, IQuest backingObject = null)
        {
            AutoCompleteTurnIns = server.GameSettings.GameServerSettings.AutoCompleteTurnInQuests;
        ...
        public override List<CDataQuestProcessState> StateMachineExecute(DdonGameServer server, GameClient client, QuestProcessState processState, PacketQueue packets, out QuestProgressState questProgressState)
        {
            ...
            // Auto-completing turn-in quests: the client auto-advanced past this delivery block
            // because the required items were in the bag, so consume them here as a normal hand-in.
            if (AutoCompleteTurnIns && process.Blocks.TryGetValue(processState.BlockNo, out var completedBlock)
                && (completedBlock.BlockType == QuestBlockType.DeliverItems
                    || completedBlock.BlockType == QuestBlockType.NewDeliverItems
                    || completedBlock.BlockType == QuestBlockType.DeliverItemsLight))
            {
                foreach (var delivery in completedBlock.DeliveryRequests)
                {
                    var consumeResult = server.ItemManager.ConsumeItemByIdFromItemBag(server, client.Character, (uint)delivery.ItemId, delivery.Amount);
        ...
        private static CDataQuestProcessState BlockAsCDataQuestProcessState(GenericQuest quest, QuestBlock questBlock)
        {
            ...
            switch (questBlock.BlockType)
            {
                ...
                case QuestBlockType.DeliverItems:
                    {
                        if (AutoCompleteTurnIns)
                        {
                            // Auto-complete the turn-in from the player's bag instead of an NPC hand-in.
