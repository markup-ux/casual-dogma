...
namespace Arrowgene.Ddon.GameServer.Characters
{
    public class WalletManager
    {
        ...
        {
            // Gold has been made useless: redirect any gold reward into character/job EXP (1:1)
            // instead of adding it to the wallet. Other currencies are unaffected.
            if (Type == WalletType.Gold && Server.GameSettings.GameServerSettings.ConvertGoldRewardsToExp)
            {
                uint totalGold = Amount + BonusAmount;
                var goldClient = Server.ClientLookup.GetClientByCharacterId(Character.CharacterId);
                if (goldClient != null && totalGold > 0)
                {
                    Server.ExpManager.AddExp(goldClient, goldClient.Character, (Amount, BonusAmount), RewardSource.Quest, connectionIn: connectionIn).Send();
                }

                CDataWalletPoint GoldWallet = Character.WalletPointList.Single(wp => wp.Type == Type);
                return new CDataUpdateWalletPoint()
                {
                    Type = Type,
                    AddPoint = 0,
                    ExtraBonusPoint = 0,
                    Value = GoldWallet.Value
                };
            }
        ...
        public CDataUpdateWalletPoint RemoveFromWallet(Character Character, WalletType Type, uint Amount, DbConnection? connectionIn = null)
        {
            ...
            // Gold has been made useless: never deduct gold for any cost. Return a no-op update
            // (no change in balance) so callers treat the spend as successful and free.
            if (Type == WalletType.Gold && Server.GameSettings.GameServerSettings.MakeGoldFree)
            {
                return new CDataUpdateWalletPoint()
                {
                    Type = Type,
