...
namespace Arrowgene.Ddon.GameServer.Characters
{
    public class JobManager
    {
        ...
        public PacketQueue SetJob(GameClient client, CharacterCommon common, JobId jobId, DbConnection? connectionIn = null)
        {
            ...
            if (common is Character && Server.GameSettings.GameServerSettings.GrantStarterWeaponsOnJobChange)
            {
                updateCharacterItemNtc.UpdateItemList.AddRange(
                    TryGrantStarterWeapons(client, (Character)common, jobId, connectionIn));
            }
        ...
        public PacketQueue UnlockLearnedNormalSkill(AssetRepository assetRepo, IDatabase database, GameClient client, CharacterCommon character, JobId job, uint skillIndex)
        {
            ...
            // When the "unlock all skills at level 1" setting is enabled, core skills can be learned
            // regardless of the character's job level.
            bool ignoreLevelRequirement = Server.GameSettings.GameServerSettings.UnlockAllSkillsAtLevelOne;
            if (characterJobData.JobPoint < Skill.JpCost || (!ignoreLevelRequirement && characterJobData.Lv < Skill.RequiredLevel))
            {
        ...
        /// <summary>
        /// Grants every learnable core skill, custom skill, EX skill and augment for all jobs to the
        /// given character (or pawn) at rank/level 1, persisting to the database. Existing learned
        /// entries are never downgraded, so any rank the player has already raised with JP is preserved.
        /// No job points are spent. Intended to be called on login/character-load and pawn creation when
        /// the "UnlockAllSkillsAtLevelOne" setting is enabled, so that everything is usable from level 1
        /// while players can still spend JP to advance individual skills/augments.
        /// </summary>
        public void GrantAllSkillsAtRank1(CharacterCommon character, DbConnection? connectionIn = null)
        {
            var skillData = Server.AssetRepository.SkillData;

            // Core (normal) skills for every job.
            foreach (var (job, learnedNormalSkills) in Server.AssetRepository.LearnedNormalSkillsAsset.LearnedNormalSkills)
            {
                for (int i = 0; i < learnedNormalSkills.Count; i++)
                {
                    uint index = (uint)(i + 1); // 1-based offset, matching UnlockLearnedNormalSkill
                    foreach (uint skillNo in learnedNormalSkills[i].SkillNo)
                    {
                        GrantNormalSkillRank1(character, job, index, skillNo, connectionIn);
                    }
                }
            }

            // Custom skills (including normally locked / special variants) for every job.
            foreach (var (job, skills) in skillData.Skills)
            {
                foreach (var skill in skills)
                {
                    GrantCustomSkillRank1(character, job, skill.SkillNo, connectionIn);
                }
            }

            // EX custom skills (defined across the job orb trees).
            foreach (CustomSkillId exSkillId in Server.JobOrbUnlockManager.GetAllExSkills())
            {
                GrantCustomSkillRank1(character, exSkillId.JobId(), exSkillId.ReleaseId(), connectionIn);
            }

            // Augments (per-job) plus secret / cross-job augments.
            HashSet<AbilityId> secretAbilityIds = skillData.SecretAbilityIds;
            foreach (var ability in skillData.AllAbilities)
            {
                if (secretAbilityIds.Contains(ability.AbilityNo))
                {
                    // Register the secret augment as unlocked so it also appears in the secret-augment list.
                    Server.Database.InsertSecretAbilityUnlock(character.CommonId, ability.AbilityNo, connectionIn);
                }
                UnlockSpecialAbility(character, ability.AbilityNo, connectionIn);
            }
        }

        private void GrantCustomSkillRank1(CharacterCommon character, JobId job, uint skillNo, DbConnection? connectionIn = null)
        {
            bool alreadyLearned = character.LearnedCustomSkills
                .Any(skill => skill != null && skill.Job == job && skill.SkillId == skillNo);
            if (alreadyLearned)
            {
                // Preserve any rank the player has already raised with JP.
                return;
            }
        ...
        private void GrantNormalSkillRank1(CharacterCommon character, JobId job, uint index, uint skillNo, DbConnection? connectionIn = null)
        {
            bool alreadyLearned = character.LearnedNormalSkills
                .Any(skill => skill != null && skill.Job == job && skill.SkillNo == skillNo);
            if (alreadyLearned)
            {
                return;
            }
            ...
            character.LearnedNormalSkills.Add(newSkill);
            Server.Database.InsertIfNotExistsNormalSkillParam(character.CommonId, newSkill, connectionIn);
        }
        ...
        private List<CDataItemUpdateResult> TryGrantStarterWeapons(
            GameClient client,
            Character character,
            JobId jobId,
            DbConnection? connectionIn)
        {
            PawnStartGearCsv? preset = Server.AssetRepository.PawnStartGearAsset
                .FirstOrDefault(x => x.Job == jobId);
            if (preset == null)
            {
                return [];
            }
            ...
            if (preset.Primary > 0 && !HasJobWeapon(character, jobId, EquipSlot.WepMain))
            {
                results.AddRange(Server.ItemManager.AddItem(
                    Server,
                    character,
                    true,
                    preset.Primary,
            ...
            if (preset.Secondary > 0 && !HasJobWeapon(character, jobId, EquipSlot.WepSub))
            {
                results.AddRange(Server.ItemManager.AddItem(
                    Server,
                    character,
        ...
        private bool HasJobWeapon(Character character, JobId jobId, EquipSlot slot)
        {
            if (HasJobWeaponInStorage(character.Equipment.Storage, jobId, slot))
            {
                return true;
            }
            ...
            {
                if (HasJobWeaponInStorage(storage, jobId, slot))
                {
                    return true;
                }
            }
        ...
        private bool HasJobWeaponInStorage(Storage storage, JobId jobId, EquipSlot slot)
        {
            foreach (Tuple<Item, uint>? entry in storage.Items)
            {
                ...
                if (!Server.AssetRepository.ClientItemInfos.TryGetValue((ItemId)item.ItemId, out ClientItemInfo? info))
                {
                    continue;
                }
