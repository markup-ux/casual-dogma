    ...
    public class InstanceDropItemManager
    {
        ...
        public InstanceDropItemManager(GameClient client, DdonGameServer server)
        {
            ...
            Generators = new()
            {
                ...
                new WildCraftDropGenerator(server),
                new ExplorationProgressionDropGenerator(server),
        ...
        public void Remove(StageLayoutId stageId, uint index)
        {
            if (!InstancedItems.TryGetValue(stageId, out Dictionary<uint, List<InstancedGatheringItem>>? stageItems))
            {
                return;
            }
